<template>
  <div class="search">
    ...
    <div class="shortcut-wrapper" v-show="!query" ref="shortcutWrapper">
      <scroll class="shortcut" :asyncData="shortcut" ref="scroll">
        <div>
          <div class="hot-key">
            <h1 class="title">热门搜索</h1>
            <ul>
              <li class="item" @click="addQuery(hotKey.k)" v-for="(hotKey, index) in hotKeys" :key="index">
                <span>{{hotKey.k}}</span>
              </li>
            </ul>
          </div>
          <div class="search-history" v-show="searchHistory.length">
            ...
            <history-list :searches="searchHistory"
                          @select="addQuery"
                          @delete="deleteSearchHistory"></history-list>
          </div>
        </div>
      </scroll>
    </div>
